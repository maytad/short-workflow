...
short-workflow/
  ...
  packages/
    db/        # Drizzle schema, reversible migrations, DB clients, query functions
...
`packages/db` is the database boundary and source of truth for Drizzle schema, migrations, database clients, and query functions.
...
`packages/db` uses Drizzle ORM for type-safe Postgres schema definitions, query builders, and migration SQL generation.

Package expectations:

- `drizzle-orm`
- `drizzle-kit`
- a Postgres driver compatible with Bun, such as `postgres`
- local migration runner scripts under `packages/db/scripts`
...
packages/db/
  ...
  migrations/
    0001_init/
      migration.sql   # up SQL, generated by Drizzle Kit or custom SQL
      down.sql        # manually written reverse SQL
    meta/             # Drizzle Kit snapshots and journal metadata
  scripts/
    migrate.ts        # custom reversible migration runner
```

Drizzle schema is the TypeScript source of truth for table definitions and inferred row types. Shared API/request schemas still live in `packages/shared`; `packages/db` should not export HTTP contract schemas.
...
- `db:generate` runs `drizzle-kit generate` against `packages/db/src/schema.ts`.
...
The custom migration runner should create and own an `app_migrations` table with at least:
...
`db:migrate:down` should:

1. Read the latest applied migration from `app_migrations`.
2. Require a sibling `down.sql`; missing rollback SQL should fail loudly.
3. Run `down.sql` inside a transaction when Postgres allows it.
4. Delete the migration record only after the rollback SQL succeeds.
5. Repeat for `--steps N` when requested.
...
3. Create Drizzle schema, reversible migration runner, migration scripts, and DB query package, then apply migrations to hosted Supabase with `db:migrate:up` and verify core queries.
