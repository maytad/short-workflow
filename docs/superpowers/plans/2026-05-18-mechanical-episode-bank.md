# Mechanical Episode Bank Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Convert the `tiny_mechanisms` prompt preset from a broad topic list into a curated mechanical episode bank with active/legacy seeds, stronger anti-repetition metadata, and mechanical-first prompt behavior.

**Architecture:** Keep the change inside the prompt layer. `packages/ai/src/prompts/presets/tinyMechanisms.ts` owns the new seed model, active seed list, legacy seed compatibility, and selection behavior. `scriptPlan.ts`, `imagePrompt.ts`, and `review.ts` consume the richer seed concept without changing the database, API, worker job schema, frontend UI, or Remotion input contract.

**Tech Stack:** Bun workspaces, TypeScript, Bun test, existing `@short-workflow/ai` prompt registry, OpenAI structured script plan prompt, Gemini/OpenAI image prompt compilation.

---

## File Structure

- Create `packages/ai/src/prompts/mechanicalEpisodeBank.test.ts`
  - Tests active-only selection, legacy seed resolution, active seed ordering, script prompt fields, and image prompt materiality.
- Modify `packages/ai/src/prompts/presets/tinyMechanisms.ts`
  - Expands `TinyMechanismsSeed`, adds active and legacy seed arrays, filters selection to active seeds, keeps old seed ids resolvable.
- Modify `packages/ai/src/prompts/scriptPlan.ts`
  - Bumps prompt version, changes channel bible usage and developer rules, sends new seed fields to the model.
- Modify `packages/ai/src/prompts/imagePrompt.ts`
  - Bumps prompt version and adds a mechanical materiality section.
- Modify `packages/ai/src/prompts/review.ts`
  - Uses active mechanical seeds instead of the old `recorded_voice` seed and emits three representative prompt samples.

No database, API, frontend, worker, Remotion, or migration files should be changed.

---

## Task 1: Add Mechanical Episode Bank Tests
...
Expected: failure because `TINY_MECHANISMS_ACTIVE_SEEDS`, `selectionStatus`, the script prompt version `5`, and image prompt version `4` do not exist yet.
...
```bash
git add packages/ai/src/prompts/mechanicalEpisodeBank.test.ts
git commit -m "test: cover mechanical episode bank prompts"
```

---

## Task 2: Expand The Tiny Mechanisms Seed Model

**Files:**
- Modify: `packages/ai/src/prompts/presets/tinyMechanisms.ts`

- [ ] **Step 1: Replace the seed type with the mechanical concept model**

In `packages/ai/src/prompts/presets/tinyMechanisms.ts`, replace the current `TinyMechanismsSeed` type with:
...
- [ ] **Step 2: Update the channel bible**

In the `TINY_MECHANISMS_CHANNEL_BIBLE` array, replace the promise, format, allowed topics, image direction, and retention lines with:
...
- [ ] **Step 3: Add the active mechanical seed array**

Replace the current `export const TINY_MECHANISMS_SEEDS: TinyMechanismsSeed[] = [` declaration with this active array first:

```ts
export const TINY_MECHANISMS_ACTIVE_SEEDS: TinyMechanismsSeed[] = [
  {
    seedId: "click_pen_cam_lock",
  ...
  },
  {
    ...
    loopPayoff: "The slider is not just closing fabric. It is building a tiny bridge one tooth at a time.",
...
- [ ] **Step 4: Add the legacy seed helper and legacy seed array**

Below `TINY_MECHANISMS_ACTIVE_SEEDS`, add:
...
- [ ] **Step 5: Update active-only seed selection**

Replace `pickNextTinyMechanismsSeed` with:
...
Leave `getTinyMechanismsSeed` searching `TINY_MECHANISMS_SEEDS` so old projects can still resolve legacy seed ids.

- [ ] **Step 6: Run the mechanical episode bank test**
...
Expected: seed selection tests pass; script/image prompt version tests still fail until later tasks.

- [ ] **Step 7: Commit the seed model changes**

Run:

```bash
git add packages/ai/src/prompts/presets/tinyMechanisms.ts packages/ai/src/prompts/mechanicalEpisodeBank.test.ts
git commit -m "feat: add mechanical episode seed bank"
```

---

## Task 3: Update Script Prompt For Mechanical Episode Concepts

**Files:**
- Modify: `packages/ai/src/prompts/scriptPlan.ts`

- [ ] **Step 1: Bump the script plan prompt version**
...
- [ ] **Step 2: Strengthen the developer rules**

In the developer message content array, after:
...
- [ ] **Step 3: Update default style context**

In `defaultProjectStyleContext`, replace the returned values with:
...
- [ ] **Step 4: Replace the user message seed fields**

In the `user` message content array, replace:
...
- [ ] **Step 5: Run the mechanical episode bank test**
...
Expected: seed selection and script prompt tests pass; image prompt version/materiality test still fails until Task 4.

- [ ] **Step 6: Commit the script prompt changes**

Run:

```bash
git add packages/ai/src/prompts/scriptPlan.ts packages/ai/src/prompts/mechanicalEpisodeBank.test.ts
git commit -m "feat: compile mechanical episode script prompts"
```

---

## Task 4: Add Mechanical Materiality To Image Prompts

**Files:**
- Modify: `packages/ai/src/prompts/imagePrompt.ts`

- [ ] **Step 1: Bump the image prompt version**
...
- [ ] **Step 2: Add the mechanical materiality prompt section**

In the `prompt` array, after the `COMPOSITION` section and before `"CAMERA AND LIGHT"`, add:
...
- [ ] **Step 3: Tighten the constraints line**
...
- [ ] **Step 4: Run the mechanical episode bank test**
...
- [ ] **Step 5: Commit the image prompt changes**

Run:

```bash
git add packages/ai/src/prompts/imagePrompt.ts packages/ai/src/prompts/mechanicalEpisodeBank.test.ts
git commit -m "feat: add mechanical image prompt materiality"
```

---

## Task 5: Update Prompt Review For Representative Mechanical Seeds

**Files:**
- Modify: `packages/ai/src/prompts/review.ts`

- [ ] **Step 1: Replace the single recorded-voice review with three mechanical samples**

Replace the entire contents of `packages/ai/src/prompts/review.ts` with:
...
Expected: JSON output contains three `reviews` entries for `click_pen_cam_lock`, `watch_escapement_ticks`, and `spray_bottle_check_valves`.

- [ ] **Step 3: Commit the review helper change**

Run:

```bash
git add packages/ai/src/prompts/review.ts
git commit -m "chore: review representative mechanical prompts"
```
...
- [ ] **Step 1: Run the AI package prompt tests**
...
- [ ] **Step 2: Run the AI package typecheck**
...
- [ ] **Step 3: Run the prompt review helper**
...
The real output contains full compiled prompt payloads; only the important shape is shown here.
...
Expected: only these implementation files are changed or newly created:
...
Other pre-existing dirty files may still appear in `git status --short`; do not stage or modify them for this plan.

- [ ] **Step 5: Commit any final verification-only adjustment**

If no files changed during final verification, do not create a commit. If a final correction was needed, stage only the files in this plan and commit:

```bash
git add packages/ai/src/prompts/mechanicalEpisodeBank.test.ts packages/ai/src/prompts/presets/tinyMechanisms.ts packages/ai/src/prompts/scriptPlan.ts packages/ai/src/prompts/imagePrompt.ts packages/ai/src/prompts/review.ts
git commit -m "fix: finalize mechanical episode bank prompts"
```
