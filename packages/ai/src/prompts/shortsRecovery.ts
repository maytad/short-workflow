...
export const SHORTS_RECOVERY_CHANNEL_BIBLE_LINES = [
  ...
  "Freshness warning: avoid solving novelty by moving to the nearest adjacent object cluster with the same hook shape or visual proof mode.",
...
export const SHORTS_RECOVERY_RESEARCH_RULES = [
  ...
  "The first frame must create a visible consequence, contradiction, or question in the first 0.5 seconds; motion and tension are useful tools, not the default answer.",
  ...
  `Deprioritize ${SHORTS_RECOVERY_PAUSED_TOPICS.join(", ")} only when they become broad lectures, weak visuals, or repeated angles; do not treat the pause list as a permanent category ban.`,
  "Start from a familiar everyday situation with a visible surprise, not from a broad fact or clean explanatory category.",
...
export const SHORTS_RECOVERY_JUDGE_RULES = [
  ...
  "Reward a first frame with a visible consequence, contradiction, or question that makes the viewer ask why before narration matters.",
  "Penalize nearest-adjacent repetition, calm product shots, clean diagrams, weak visuals, and broad lectures.",
  "Do not over-reward pressure, valve, cap, pump, latch, or spring ideas just because their motion is easy to show.",
...
export const SHORTS_RECOVERY_SCRIPT_RULES = [
  ...
  ...SHORTS_RECOVERY_METRIC_GATES,
  "The first scene must open on visible action, contrast, texture change, reflection, deformation, sound-source clue, residue, state change, failure, or contradiction already happening.",
  "The first caption must be no more than 4 words and must read like a feed hook, not a transcript line.",
  "The first caption must also work as a visual headline baked into the generated hook image: plain English words, no punctuation, no labels, no explanation.",
  "Prefer contradiction headline structures for scene 1: BACKWARD BUT FORWARD, SHOULD LOCK, STAYS SHUT, WHY DID IT SLIP.",
...
export const SHORTS_RECOVERY_IMAGE_RULES = [
  ...
  "Show a visible consequence already happening: action, contrast, texture change, reflection shift, deformation, residue, state change, resistance, or failure.",
  "The viewer should understand the object and curiosity gap from the image itself; a hook headline may sharpen the contradiction but cannot carry a weak visual.",
] as const;
